...
export default function ExportDiklatInternalModal({ ...props }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  useBackOnClose(`export-diklat-internal-modal-${1}`, isOpen, onOpen, onClose);
  ...
  const handleExport = () => {
    ...
    req
      .post("/api/rski/dashboard/perusahaan/diklat-internal/export", payload, {
        responseType: "blob",
      })
      .then((r) => {
        if (r?.status === 200) {
          download(
            r.data,
            `Data Diklat Internal ${formatDate(
              dateRange?.tgl_mulai,
              "short",
