...
import FilterKaryawanMedis from "./FilterKaryawanMedis";

interface Props extends ButtonProps {}

export default function ExportKaryawanMedisModal({ ...props }: Props) {
  ...
  const handleExport = () => {
    ...
    const url = `/api/rski/dashboard/karyawan/export-karyawan-medis`;

    const payload = {
      masa_sip: filterConfig.masa_sip,
      masa_str: filterConfig.masa_str,
    };
    ...
    req
      ...
      .then((r) => {
        if (r.status === 200) {
          download(r.data, `Export Karyawan Medis`, "xls");
        } else {
          toast({
            status: "error",
            title:
