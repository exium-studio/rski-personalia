...
import EditSpesialisasiDokterModalDisclosure from "../independent/EditSpesialisasiDokterModalDisclosure";
...
export default function TabelPengaturanSpesialisasiDokter({
  filterConfig,
}: Props) {
  // SX
  ...
  const rowOptions = [
    (rowData: any) => {
      return (
        <EditSpesialisasiDokterModalDisclosure rowData={rowData}>
        ...
        </EditSpesialisasiDokterModalDisclosure>
      );
    },
    (rowData: any) => {
      return (
        <RestoreDataPengaturanModalDisclosure
          id={rowData.id}
          url={`api/rski/dashboard/pengaturan/spesialisasi/restore`}
        >
          <PermissionTooltip permission={editPermission} placement="left">
            <MenuItem
    ...
    (rowData: any) => {
      return (
        <DeleteDataPengaturanModalDisclosure
          id={rowData.id}
          url={`api/rski/dashboard/pengaturan/spesialisasi`}
        >
  ...
  const fd = data?.filter((item: any) => {
    ...
    const matchesSearchTerm = item?.nama_spesialisasi
      ?.toLowerCase()
      .includes(searchTerm);
  ...
  const formattedData = fd?.map((item: any) => ({
    ...
    columnsFormat: [
      {
        value: item.nama_spesialisasi,
        td: (
          <Tooltip
            openDelay={500}
            label={item.nama_spesialisasi}
            placement="right"
          >
            <Text
              w={"100%"}
