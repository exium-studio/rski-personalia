...
export default function SelectKategoriStatusKaryawan({
  name,
  onConfirm,
  inputValue,
  withSearch,
  ...
  useEffect(() => {
    if (isOpen && !options) {
      req
        .get("/api/get-list-kategori-status-karyawan")
        .then((r) => {
          if (r?.status === 200) {
  ...
  return (
    <SingleSelectModal
      id="select-kategori-status-karyawan-modal"
      name={name}
      isOpen={isOpen}
      onOpen={onOpen}
      onClose={onClose}
      ...
      isError={isError}
      placeholder={placeholder || "Pilih Kategori Status Karyawan"}
