  ...
  useEffect(() => {
    if (isOpen) {
      formikRef.current.setFieldValue("user", {
        value: rowData.originalData.list_peserta?.[0]?.user?.id,
        label: rowData.originalData.list_peserta?.[0]?.user?.nama,
      });
      ...
      formikRef.current.setFieldValue(
        "tgl_mulai",
        new Date(formatDate(rowData.originalData.tgl_mulai, "iso"))
      );
      formikRef.current.setFieldValue(
        "tgl_selesai",
        new Date(formatDate(rowData.originalData.tgl_selesai, "iso"))
      );
      formikRef.current.setFieldValue(
        "jam_mulai",
        rowData.originalData.jam_mulai
      );
      formikRef.current.setFieldValue(
        "jam_selesai",
        rowData.originalData.jam_selesai
      );
              ...
              <SimpleGrid columns={[1, 2]} spacingX={4} mb={8}>
                ...
                >
                  ...
                  <FileInputLarge
                    ...
                    placeholder="jika tidak ada perubahan sertifikat, biarkan kosong"
                  />
                  <FormErrorMessage>
